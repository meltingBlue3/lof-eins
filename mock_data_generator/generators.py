...
class NAVGenerator:
    ...
    def generate(self, ticker: str) -> pd.DataFrame:
        ...
        returns = np.random.normal(
            self.config.nav_drift,
            self.config.nav_volatility,
            n_days
        )
...
class PriceGenerator:
    ...
    def generate(self, ticker: str, nav_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(n_days):
            if not in_spike:
                ...
                if np.random.random() < self.config.spike_probability:
                    ...
                    premium_rates[i] = np.random.normal(0.0, self.config.premium_volatility)
            else:
        ...
        open_prices = close_prices * (1 + np.random.normal(0, intraday_volatility, n_days))
        ...
        volume_multiplier = 1 + np.abs(premium_rates) * 5  # 5x volume increase at high premium
        volumes = np.random.lognormal(
            np.log(base_volume) + np.log(volume_multiplier),
            0.5,
            n_days
        ).astype(int)
            ...
            if not in_limit:
                ...
                if premium > self.config.limit_trigger_threshold:
                    ...
                    if high_premium_days >= self.config.consecutive_days:
                        ...
                        if i + 1 < len(dates):
                            limit_start = pd.Timestamp(dates[i + 1]).strftime('%Y-%m-%d')
                        else:
