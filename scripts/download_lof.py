...
class RealDataDownloader:
    ...
    def __init__(self, username: str, password: str, output_dir: str, batch_size: int = 50):
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            print(f"[OK] JoinQuant 登录成功 | 剩余额度: {count['spare']}/{count['total']}")
            return True
    ...
    def get_market_data(self, codes: list, start_date: str, end_date: str) -> pd.DataFrame:
        """获取行情数据 (OHLCV)"""
        try:
            df = jq.get_price(
                codes,
    ...
    def process_and_save(self, codes: list, price_df: pd.DataFrame, nav_df: pd.DataFrame) -> list:
        """拆分数据并保存"""
        processed_tickers = []
        ...
        for code in codes:
            ...
            if not price_df.empty:
                ...
                if not df_m.empty:
                    ...
                    df_m['volume_ma5'] = df_m['volume'].rolling(window=5).mean().fillna(df_m['volume'])
                    
                    cols = ['date', 'ticker', 'open', 'high', 'low', 'close', 'volume', 'volume_ma5']
